...
- **Pages on both transports.** A `&[T]` handler mounts with `batch(n)` and gets pages capped at that size. Neither client reads several entries at a time, so the pages are assembled on the client out of the framework's own buffer - nothing at the mount site says so.
