...
- **Delayed retries through a deferred copy.** With no settlement to lean on, `HandlerOutcome::retry_after(delay)` works the one way it can: wire `b.retry_via(broker.publisher())` and the runtime republishes the message to the stream key the subscription reads once the delay is over. A `.replay()` subscription and the stdio form say up front that nothing addresses them, so a scope that defers over one refuses to start instead of losing every delayed message.
...
fn app() -> impl App {
    RustStream::new(AppInfo::new("orders", "0.1.0"))
        .with_broker(FileBroker::new("/var/lib/svc/orders.ss"), |b| {
            // `.out(marker, policy)` is the mount site's only publish verb; `Reply` names the
            // position a handler's return value leaves through.
...
async fn an_order_is_confirmed() -> Result<(), Box<dyn std::error::Error>> {
    let app = RustStream::new(AppInfo::new("orders", "0.1.0"))
        .with_broker(FileTestBroker::new(), |b| {
            b.include(confirm).out(Reply, FileTestPublish);
        });
