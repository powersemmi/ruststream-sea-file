...
- **Replayable subscriptions.** `FileStream::new(key)` starts at the live tail; `.start(Start::Beginning)` replays everything retained first; `.replay()` reads a finished file and completes the stream when it ends - batch-style processing of a recorded log.
