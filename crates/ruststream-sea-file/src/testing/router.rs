//! Subscription registry and fanout for the in-process file-stream stand-in.
//!
//! Core routing only: an exact-name match fans a published message out to every live
//! subscription on that name, and a per-name log records traffic for assertions. The file
//! transport's own semantics (persistence, beacons, replay, repositioning) are transport
