...
(() => {
  ...
  // as the document wrote them; the page adds no words of its own to them. `storage` is this
  // crate's own group: there is no broker here, so what a number depends on is the filesystem
  // under the stream file rather than a server.
  const ENVIRONMENT = [
    ["machine", ["cpu", "architecture", "cpu_frequency", "cores", "memory", "memory_speed"]],
    ["os", ["os"]],
    ["broker", ["broker"]],
    ["storage", ["filesystem", "file_size", "page_cache", "round_trip"]],
